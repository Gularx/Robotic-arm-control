...
#define DHD_DEVICE_DELTA3          63       /// Force Dimension DELTA.3触觉设备(USB版本)的设备标识符。
...
#define DHD_DEVICE_DELTA6          64       /// Force Dimension DELTA.6触觉设备(USB版本)的设备标识符
#define DHD_DEVICE_SIGMA33P       106       /// 右手版本的 Force Dimension SIGMA.6+触觉设备的设备标识符
#define DHD_DEVICE_SIGMA33P_LEFT  107       /// 左手版本的 Force Dimension SIGMA.6+触觉设备的设备标识符
...
/* encoder count */
#define DHD_MAX_DOF                 8       /// 可用编码器通道的最大数量

/* delta motor index */
#define DHD_DELTA_MOTOR_0           0       /// DELTA 结构的电机 0 的阵列索引（仅限专家模式）
#define DHD_DELTA_MOTOR_1           1       /// DELTA 结构的电机 1 的阵列索引（仅限专家模式）
#define DHD_DELTA_MOTOR_2           2       /// DELTA 结构的电机 3 的阵列索引（仅限专家模式）

/* delta encoder index */
#define DHD_DELTA_ENC_0             0       /// DELTA 结构的编码器 0 的数组索引（仅限专家模式）
#define DHD_DELTA_ENC_1             1       /// DELTA 结构的编码器 1 的数组索引（仅限专家模式）
#define DHD_DELTA_ENC_2             2       /// DELTA 结构的编码器 2 的数组索引（仅限专家模式）

/* wrist motor index */
#define DHD_WRIST_MOTOR_0           3       /// WRIST 结构的电机 0 的阵列索引（仅限专家模式）
#define DHD_WRIST_MOTOR_1           4       /// WRIST 结构的电机 1 的阵列索引（仅限专家模式）
#define DHD_WRIST_MOTOR_2           5       /// WRIST 结构的电机 2 的阵列索引（仅限专家模式）

/* wrist encoder index */
#define DHD_WRIST_ENC_0             3       /// WRIST结构的编码器0的数组索引（仅限专家模式）
#define DHD_WRIST_ENC_1             4       /// WRIST结构的编码器1的数组索引（仅限专家模式）
#define DHD_WRIST_ENC_2             5       /// WRIST结构的编码器2的数组索引（仅限专家模式）
...
#define DHD_TIMEGUARD               1       /// TimeGuard功能阻止与设备进行不必要的通信时使用的返回值
#define DHD_MOTOR_SATURATED         2       /// 当至少一个电机无法提供所需扭矩时使用的返回值。电机组按比例缩放，以保持力和扭矩方向超过大小

/* status count */
#define DHD_MAX_STATUS             17       /// 状态数组的长度。有关详细信息，请参阅 device status


/* status codes */
#define DHD_STATUS_POWER            0       /// 状态数组中电源标志的索引。此标志指示设备是否通电
#define DHD_STATUS_CONNECTED        1       /// 状态数组中连接标志的索引。此标志指示设备是否已连接。
#define DHD_STATUS_STARTED          2       /// 状态数组中开始标志的索引。此标志指示device controller是否正在运行
#define DHD_STATUS_RESET            3       /// 状态数组中RESET标志的索引。此标志指示设备是否处于重置模式。有关详细信息，请参阅Device Modes
#define DHD_STATUS_IDLE             4       /// 状态数组中IDLE标志的索引。此标志指示设备是否处于IDLE模式。有关详细信息，请参阅device modes
#define DHD_STATUS_FORCE            5       /// 状态数组中FORCE标志的索引。此标志指示设备是否处于FORCE模式。有关详细信息，请参阅Device Modes
#define DHD_STATUS_BRAKE            6       /// 状态数组中BRAKE标志的索引。此标志指示设备是否处于制动模式。有关详细信息，请参阅设备模式
#define DHD_STATUS_TORQUE           7       /// 状态数组中TORQUE标志的索引。当设备处于FORCE（力）模式时，此标志指示扭矩是否激活。有关详细信息，请参阅Device Modes
#define DHD_STATUS_WRIST_DETECTED   8       /// 状态数组中WRIST_DETECTED标志的索引。此标志指示设备是否有手腕。有关详细信息，请参阅device types
#define DHD_STATUS_ERROR            9       /// 状态数组中错误标志的索引。此标志指示 device controller 上是否发生错误。
#define DHD_STATUS_GRAVITY         10       /// 状态数组中重力标志的索引。此标志指示重力补偿选项是否启用。
#define DHD_STATUS_TIMEGUARD       11       /// 状态数组中TimeGuard标志的索引。此标志指示TimeGuard功能是否已启用。有关详细信息，请参阅TimeGuard功能。
#define DHD_STATUS_WRIST_INIT      12       /// 状态数组中WRIST_INIT标志的索引。此标志指示设备手腕是否已初始化。有关详细信息，请参阅device types
#define DHD_STATUS_REDUNDANCY      13       /// 冗余编码器一致性检查的状态。对于配备冗余编码器的设备，值为1表示冗余检查成功。否则，或者如果设备没有冗余编码器，则报告值为 0。
#define DHD_STATUS_FORCE_OFF_CAUSE 14       /// DHD_STATUS_FORCEOFFCAUSE
#define DHD_STATUS_LOCKS           15
#define DHD_STATUS_AXIS_CHECKED    16

/* buttons count */
#define DHD_MAX_BUTTONS            16       /// SDK可以在Force Dimension触觉设备上寻址的最大按钮数

/* velocity estimator computation mode */
#define DHD_VELOCITY_WINDOWING      0       /// 默认速度估算器模式。在这种模式下，通过将当前位置与给定时间间隔前的位置进行比较来估计速度。这个时间间隔（或“窗口”）可以使用dhdConfigLinearVelocity进行调整，并且应该进行修改，以最适合给定应用程序的设备动态行为。开窗估计器模式是资源密集度最低的模式。
#define DHD_VELOCITY_INSTANT        2
#define DHD_VELOCITY_WINDOW        20  // [ms]  速度估计器使用的默认窗口大小。实际时间间隔（或“窗口”）可以使用dhdConfigLinearVelocity进行调整，并且应该进行修改，以最适合给定应用程序的设备动态行为。

/* USB operation modes */
#define DHD_COM_MODE_SYNC           0       /// 同步USB模式按顺序执行USB读取和写入操作，允许4kHz的理论触觉刷新率。请注意，其他因素也会影响USB性能，包括操作系统的选择、机器负载和程序优化。
#define DHD_COM_MODE_ASYNC          1       /// 异步USB模式是默认模式。异步USB模式允许操作系统并行进行USB端口上的读写操作。这种并行操作通过减少通信抖动来提高刷新率的稳定性。其他因素也会影响USB性能，包括操作系统的选择、机器负载和程序优化。
#define DHD_COM_MODE_VIRTUAL        3       /// 连接到虚拟设备时会报告此模式
#define DHD_COM_MODE_NETWORK        4       /// 当使用力维度网络连接模式连接到触觉设备时，会报告此模式。

/* causes for device FORCE OFF state */
#define DHD_FORCE_OFF_NONE          0       /// 还没有什么导致部队被关闭
#define DHD_FORCE_OFF_BUTTON        1       /// 强制按钮被按下
#define DHD_FORCE_OFF_VELOCITY      2       /// 达到了速度阈值 velocity threshold
#define DHD_FORCE_OFF_WATCHDOG      3       /// 通信监察机构communication watchdog介入了
#define DHD_FORCE_OFF_SOFTWARE      4       /// 软件请求关闭强制，例如dhdEnableForce（）
#define DHD_FORCE_OFF_USBDISCN      5       /// USB电缆已断开
#define DHD_FORCE_OFF_DEADMAN       6       /// DHD强行关闭死囚

/* thread management */
#define DHD_THREAD_PRIORITY_DEFAULT 0       /// 这个常量可以用来告诉dhdStartThread（）函数为新创建的线程使用默认的操作系统优先级
#define DHD_THREAD_PRIORITY_HIGH    1       /// 此常量可用于告诉dhdStartThread（）函数为新创建的线程使用高于默认操作系统优先级的优先级
#define DHD_THREAD_PRIORITY_LOW     2       /// 这个常数可以用来告诉dhdStartThread（）函数使用比新创建的线程的默认操作系统优先级更低的优先级
