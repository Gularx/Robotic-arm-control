...
//    m_pApi->Service_Get_Current_Arm_State(ArmSocket, joint, &pose, &arm_Err, &sys_Err);
    ...
    // message
    std::cout << "Force Dimension - Auto Center Gravity " << dhdGetSDKVersionStr() << std::endl;
    ...
    if (!drdIsInitialized() && drdAutoInit() < 0) {
        std::cout << "error: auto-initialization failed (" << dhdErrorGetLastStr() << ")" << std::endl;
        dhdSleep(2.0);
        return;
    } else if (drdStart() < 0) {
        std::cout << "error: regulation thread failed to start (" << dhdErrorGetLastStr() << ")" << std::endl;
        dhdSleep(2.0);
        return;
    }
    ...
    std::cout << "init_ (" << init_px << " " << init_py << " " << init_pz << ") m | rad (" << init_oa << " " << init_ob
              << " " << init_og << ")" << std::endl;
    ...
    while (!done) {
        ...
        if ((t1 - t0) > REFRESH_INTERVAL) {
            ...
            std::cout << "cur_ (" << cur_px << " " << cur_py << " " << cur_pz << ") m | rad (" << cur_oa << " "
                      << cur_ob << " " << cur_og << ")" << std::endl;
            ...
            transformedVector[0] =
                    rotationMatrix[0][0] * diff_px + rotationMatrix[0][1] * diff_py + rotationMatrix[0][2] * diff_pz;
            transformedVector[1] =
                    rotationMatrix[1][0] * diff_px + rotationMatrix[1][1] * diff_py + rotationMatrix[1][2] * diff_pz;
            transformedVector[2] =
                    rotationMatrix[2][0] * diff_px + rotationMatrix[2][1] * diff_py + rotationMatrix[2][2] * diff_pz;
